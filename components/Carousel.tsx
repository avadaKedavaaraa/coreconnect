...
import { Download, Heart, Sparkles, Trash2, ChevronLeft, ChevronRight, Pin, Video, Layers, Clock } from 'lucide-react';
...
const Carousel: React.FC<CarouselProps> = ({ items, lineage, onExtract, isAdmin, onDelete }) => {
  ...
  const displayItems = useMemo(() => {
    ...
    filtered.sort((a, b) => {
        ...
        if (dateA === dateB) return (a.order_index || 0) - (b.order_index || 0);
        return dateB - dateA;
    });
    ...
    return filtered.filter(item => {
        ...
        if (filter === 'new') {
            ...
            return topTwoDates.includes(itemDate);
        }
        return true;
    });
  }, [localItems, filter]);

  const safeLength = displayItems.length || 1;
  ...
  const onTouchEnd = (e: React.TouchEvent) => {
    ...
    if (isLeftSwipe) setActiveIndex((prev) => (prev + 1) % safeLength);
    if (isRightSwipe) setActiveIndex((prev) => (prev - 1 + safeLength) % safeLength);
  };
  ...
  return (
    <div className="flex flex-col items-center justify-center w-full gap-8 py-4">
      ...
      {/* FILTER BAR */}
      <div className="z-40 flex gap-2 p-1.5 rounded-full bg-black/40 backdrop-blur-md border border-white/10 shadow-xl transition-all hover:bg-black/60">
        {[
          ...
          { id: 'new', label: 'New', icon: Sparkles },
           ...
           <button
             ...
             className={`px-4 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-wider flex items-center gap-2 transition-all duration-300
               ${filter === f.id 
                 ? (lineage === Lineage.WIZARD ? 'bg-emerald-600 text-black shadow-[0_0_15px_rgba(16,185,129,0.4)] scale-105' : 'bg-fuchsia-600 text-black shadow-[0_0_15px_rgba(217,70,239,0.4)] scale-105') 
      ...
      {displayItems.length === 0 && (
          <div className="h-[450px] w-full flex flex-col items-center justify-center opacity-50">
             <div className="text-4xl mb-2">🤷‍♂️</div>
             <p className="text-sm font-mono text-white/50">No {filter} items found.</p>
          </div>
      )}
          ...
          >
            {displayItems.map((item, index) => {
              ...
              return (
                <div
                  key={item.id}
                  className={`group absolute top-0 left-0 w-full h-full rounded-2xl flex flex-col justify-between p-6 transition-all duration-500 cursor-pointer overflow-hidden border backdrop-blur-md
                    ...
                    ${isActive 
                        ? (lineage === Lineage.WIZARD 
                            ? 'opacity-100 z-50 scale-105 shadow-[0_0_20px_rgba(16,185,129,0.3)] border-emerald-500 pointer-events-auto' 
                            : 'opacity-100 z-50 scale-105 shadow-[0_0_20px_rgba(217,70,239,0.3)] border-fuchsia-500 pointer-events-auto')
                        : (isMobile 
                ...
                >
                  {hasImage && (
                    <div className="absolute inset-0 z-0 pointer-events-none overflow-hidden rounded-2xl">
                      <img src={item.image} alt={item.title} className="w-full h-full object-cover opacity-40 transition-transform duration-700 group-hover:scale-110" />
                  ...
                  <div className="relative z-10 w-full h-full flex flex-col select-none">
                      ...
                      {/* BODY - SCROLLABLE CONTENT WITH TIGHTER LINES */}
                      <div className="flex-1 flex flex-col overflow-hidden mt-2 relative">
                          <h2 
                            className={`text-3xl font-bold leading-tight mb-2 drop-shadow-md shrink-0`}
                            style={titleStyle}
                          >
                              {item.title || 'Untitled'}
                          </h2>
                      ...
                      {/* FOOTER */}
                      <div className="mt-4 pt-4 border-t border-white/10 flex items-center justify-between w-full pointer-events-auto shrink-0 z-20">
                          <button 
                          ...
                          {isActive && (
                              <button
                                ...
                                className={`flex items-center gap-2 px-6 py-2 rounded-lg font-bold text-[10px] uppercase tracking-widest transition-all hover:scale-105 active:scale-95 border backdrop-blur-md shadow-lg
                                    ${lineage === Lineage.WIZARD 
