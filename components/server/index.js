...
(async function init() {
    try {
        ...
        // Ensure visitor log table exists (Optional check if you don't have migrations)
        // await supabase.rpc('create_visitor_table_if_not_exists'); 
...
app.post('/api/visitor/heartbeat', async (req, res) => {
    ...
    // Anonymize IP for privacy compliance but keep hash for unique counts if needed
    const ip = req.headers['x-forwarded-for'] || req.socket.remoteAddress;
    const ipHash = crypto.createHash('sha256').update(ip || 'unknown').digest('hex').substring(0, 10);

    try {
        // Upsert into visitors table
        // Requires a table 'visitor_logs' with columns: visitor_id (PK), display_name, total_time_spent, visit_count, last_active, ip_hash
        ...
        // Don't crash app if DB table missing, just warn
        console.warn("Visitor Log Error:", e.message);
