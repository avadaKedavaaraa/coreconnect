...
import { Lineage, type CarouselItem, type LectureRule, GlobalConfig, Sector, SECTORS } from '../types';
import { 
    ...
    CalendarDays, Clock, Users, MousePointer2, Columns 
} from 'lucide-react';
...
const isDayMatch = (checkDate: Date, days?: string[], legacyDay?: string) => {
    ...
    if (days && days.length > 0) return days.includes(dayName);
...
export const SectorView: React.FC<SectorViewProps> = ({
    ...
    const [viewMode, setViewMode] = useState<'folders' | 'masonry' | 'list' | 'grid' | 'columns'>('folders');
    ...
    // --- DATA PROCESSING (Exact Original Logic) ---
    useEffect(() => {
        let combined = [...items];
        combined.sort((a, b) => {
    ...
    useEffect(() => {
        ...
        // Default to 'columns' (Original UI) for lectures, 'folders' for everything else
        setViewMode(isLectures ? 'columns' : 'folders');
        // Default Lecture Date to Today (IST)
    ...
    const getClassesForDate = (dateStr: string, batch?: string) => {
        ...
        return schedules.filter(rule => {
            ...
            if (batch && rule.batch && rule.batch !== batch && rule.batch !== 'AICS') return false; // AICS usually default/shared
    ...
    // 1. Original Side-by-Side (Columns)
        ...
        const renderCard = (cls: LectureRule, color: 'blue' | 'fuchsia') => (
            <div key={cls.id} className={`group relative rounded-xl overflow-hidden border border-white/10 flex flex-col transition-all duration-300 hover:scale-[1.02] hover:shadow-2xl bg-[#121212]`}>
                <div className="h-28 w-full relative overflow-hidden">
                    {cls.image ? ( <img src={cls.image} alt={cls.subject} className="w-full h-full object-cover opacity-80 group-hover:opacity-100 transition-opacity" /> ) : ( <div className={`w-full h-full flex items-center justify-center opacity-20 bg-gradient-to-br ${color === 'blue' ? 'from-blue-900 to-black' : 'from-fuchsia-900 to-black'}`}><Video size={32} /></div> )}
                    <div className="absolute inset-0 bg-gradient-to-t from-[#121212] via-transparent to-transparent"></div>
                    <div className={`absolute top-3 right-3 px-2 py-1 rounded text-[10px] font-bold uppercase tracking-wider bg-black/60 backdrop-blur border border-white/10 text-white shadow-lg`}>{cls.startTime}</div>
                </div>
                <div className="p-4 flex-1 flex flex-col relative -mt-6">
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center border-2 border-[#121212] shadow-lg mb-2 z-10 ${color === 'blue' ? 'bg-blue-600 text-white' : 'bg-fuchsia-600 text-white'}`}><Video size={18} /></div>
                    <h3 className="text-lg font-bold text-white leading-tight mb-1">{cls.subject}</h3>
                    <p className="text-xs text-white/60 mb-4 line-clamp-2 min-h-[2.5em]">{cls.customMessage || "No class notes."}</p>
                    {cls.link && ( <a href={cls.link} target="_blank" rel="noreferrer" className={`mt-auto w-full py-2.5 rounded font-bold text-[10px] uppercase tracking-widest text-center transition-all border ${color === 'blue' ? 'bg-blue-900/20 text-blue-300 border-blue-500/30 hover:bg-blue-600 hover:text-white' : 'bg-fuchsia-900/20 text-fuchsia-300 border-fuchsia-500/30 hover:bg-fuchsia-600 hover:text-white'}`}>Join Session</a> )}
                </div>
            </div>
        );
        ...
        return (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 animate-[fade-in_0.3s]">
                <div className="space-y-4">
                    <div className="flex items-center justify-between bg-blue-950/20 p-3 rounded-lg border border-blue-500/20"><div className="flex items-center gap-3"><div className="h-8 w-1 bg-blue-500 rounded-full"></div><div><h3 className="font-bold text-blue-100 leading-none">AICS</h3><div className="text-[10px] text-blue-300/60 uppercase tracking-wider">Artificial Intelligence</div></div></div><span className="text-[10px] font-mono bg-blue-900/40 text-blue-300 px-2 py-1 rounded border border-blue-500/20">{aicsClasses.length} Events</span></div>
                    <div className="grid gap-4">{aicsClasses.length > 0 ? aicsClasses.map(c => renderCard(c, 'blue')) : <div className="text-center py-8 opacity-40 text-xs border border-dashed border-white/10 rounded">No AICS classes.</div>}</div>
                </div>
                <div className="space-y-4">
                    <div className="flex items-center justify-between bg-fuchsia-950/20 p-3 rounded-lg border border-fuchsia-500/20"><div className="flex items-center gap-3"><div className="h-8 w-1 bg-fuchsia-500 rounded-full"></div><div><h3 className="font-bold text-fuchsia-100 leading-none">CSDA</h3><div className="text-[10px] text-fuchsia-300/60 uppercase tracking-wider">Data Analytics</div></div></div><span className="text-[10px] font-mono bg-fuchsia-900/40 text-fuchsia-300 px-2 py-1 rounded border border-fuchsia-500/20">{csdaClasses.length} Events</span></div>
                    <div className="grid gap-4">{csdaClasses.length > 0 ? csdaClasses.map(c => renderCard(c, 'fuchsia')) : <div className="text-center py-8 opacity-40 text-xs border border-dashed border-white/10 rounded">No CSDA classes.</div>}</div>
                </div>
            </div>
        );
    };

    // 2. New Unified List/Grid (Large Images + Animation)
            ...
            <div className={`grid gap-6 animate-[fade-in_0.3s] ${viewMode === 'list' ? 'grid-cols-1' : 'grid-cols-1 md:grid-cols-2 lg:grid-cols-3'}`}>
                {activeClasses.length > 0 ? (
                    activeClasses.map(cls => (
                        <div key={cls.id} className={`group relative rounded-xl overflow-hidden border border-white/10 flex flex-col transition-all duration-300 hover:-translate-y-2 hover:shadow-[0_20px_40px_rgba(0,0,0,0.6)] bg-[#121212] cursor-pointer ring-1 ring-white/5 hover:ring-white/20`} onClick={() => window.open(cls.link, '_blank')}>
                            <div className="h-48 w-full relative overflow-hidden bg-black">
                                {cls.image ? ( <img src={cls.image} alt={cls.subject} className="w-full h-full object-cover opacity-70 group-hover:opacity-100 transition-all duration-500 transform group-hover:scale-110" /> ) : ( <div className={`w-full h-full flex items-center justify-center opacity-30 bg-gradient-to-br ${activeBatch === 'AICS' ? 'from-blue-900 via-blue-800' : 'from-fuchsia-900 via-purple-900'} to-black`}><Video size={56} className="text-white drop-shadow-lg" /></div> )}
                                <div className="absolute inset-0 bg-gradient-to-t from-[#121212] via-transparent to-transparent"></div>
                                <div className="absolute top-3 right-3 px-3 py-1.5 rounded-lg text-xs font-bold uppercase tracking-widest bg-black/80 backdrop-blur-md border border-white/10 text-white shadow-xl flex items-center gap-2 z-10"><Clock size={12} className={activeBatch === 'AICS' ? 'text-blue-400' : 'text-fuchsia-400'} /><span>{cls.startTime}</span><span className="opacity-50">|</span><span>{cls.endTime || 'End'}</span></div>
                            </div>
                            <div className="p-5 flex-1 flex flex-col relative -mt-10 z-20">
                                <div className={`w-14 h-14 rounded-2xl rotate-3 flex items-center justify-center border-4 border-[#121212] shadow-2xl mb-3 transition-transform group-hover:rotate-6 group-hover:scale-110 ${activeBatch === 'AICS' ? 'bg-gradient-to-br from-blue-600 to-blue-800' : 'bg-gradient-to-br from-fuchsia-600 to-purple-800'}`}><Video size={24} className="text-white drop-shadow-md" /></div>
                                <h3 className="text-2xl font-bold text-white leading-tight mb-2 group-hover:text-blue-300 transition-colors">{cls.subject}</h3>
                                <div className="mb-6 flex-1"><p className="text-sm text-zinc-400 leading-relaxed border-l-2 border-white/10 pl-3 italic">{cls.customMessage || "No additional notes."}</p></div>
                                {cls.link && (
                                    <button className={`relative overflow-hidden w-full py-3.5 rounded-xl font-bold text-xs uppercase tracking-[0.2em] text-center transition-all shadow-lg flex items-center justify-center gap-3 group/btn ${activeBatch === 'AICS' ? 'bg-blue-600/10 border border-blue-500/30 text-blue-200 group-hover:bg-blue-600 group-hover:text-white group-hover:border-blue-500' : 'bg-fuchsia-600/10 border border-fuchsia-500/30 text-fuchsia-200 group-hover:bg-fuchsia-600 group-hover:text-white group-hover:border-fuchsia-500'}`}>
                                        ...
                                        <div className="absolute right-4 opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-y-4 group-hover:translate-y-0 text-white drop-shadow-lg z-20"><MousePointer2 size={24} className="fill-white text-black" /></div>
                                    </button>
                                )}
                            </div>
                        </div>
    ...
    return (
        <div className={`w-full max-w-7xl mx-auto px-4 sm:px-6 pb-20 animate-[fade-in-up_0.3s_ease-out] ${isWizard ? 'scrollbar-wizard' : 'scrollbar-muggle'}`}>
            ...
            <div className="flex flex-col md:flex-row justify-between items-end gap-4 mb-8 border-b border-white/10 pb-4">
                <div className="flex-1 w-full md:w-auto">
                    <div className="flex items-center gap-4 mb-2">
                        ...
                        {isLectures && (
                            <h2 className={`text-2xl font-bold ${isWizard ? 'font-wizardTitle text-emerald-100' : 'font-muggle text-fuchsia-100'}`}>Lecture Schedule</h2>
                        )}
                    </div>
                ...
                <div className="flex gap-2 items-center">
                    ...
                    {isLectures && (
                        <>
                             ...
                             <div className="flex bg-black/40 rounded-lg p-1 border border-white/10">
                                <button onClick={() => setViewMode('columns')} className={`p-2 rounded ${viewMode === 'columns' ? 'bg-white/20 text-white' : 'text-white/40 hover:text-white'}`} title="Columns"><Columns size={18} /></button>
                    ...
                    {isAdmin && onAddItem && <button onClick={() => onAddItem(sectorId)} className={`flex items-center gap-2 px-4 py-2 rounded-full font-bold shadow-lg transition-all hover:scale-105 active:scale-95 ${isWizard ? 'bg-emerald-600 text-black hover:bg-emerald-500 shadow-[0_0_15px_rgba(16,185,129,0.3)]' : 'bg-fuchsia-600 text-black hover:bg-fuchsia-500 shadow-[0_0_15px_rgba(217,70,239,0.3)]'}`}><Plus size={18} /> <span className="hidden sm:inline">Add Content</span></button>}
                </div>
            </div>
            ...
            {/* RENDER CONTENT */}
            {isLectures ? (viewMode === 'columns' ? renderSideBySideSchedule() : renderUnifiedSchedule()) : (
                viewMode === 'folders' && !search && !dateFilter && !showPinnedOnly ? (
