...
export const SectorView: React.FC<SectorViewProps> = ({ 
  ...
  const renderSideBySideSchedule = () => {
    ...
    const renderEmpty = (text: string) => (
        <div className="py-8 text-center border-2 border-dashed border-white/10 rounded-xl bg-white/5 opacity-50">
             <Calendar size={24} className="mx-auto mb-2 text-white/30" />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 animate-[fade-in_0.3s]">
            ...
            <div className="space-y-4">
                ...
                <div className="grid gap-6">
                    {aicsClasses.length > 0 ? aicsClasses.map(c => <LectureCard3D key={c.id} cls={c} color="blue" />) : renderEmpty("No AICS classes.")}
                </div>
            </div>
            ...
            <div className="space-y-4">
                ...
                <div className="grid gap-6">
                    {csdaClasses.length > 0 ? csdaClasses.map(c => <LectureCard3D key={c.id} cls={c} color="fuchsia" />) : renderEmpty("No CSDA classes.")}
                </div>
            </div>
        </div>
    );
