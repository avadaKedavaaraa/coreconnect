...
import {
    ...
    ChevronRight, AlertTriangle, ChevronLeft
} from 'lucide-react';
...
// ==========================================
// 🎨 COMPONENT: SWIMLANE ROW (Netflix Style)
// ==========================================
const SwimlaneRow = ({ title, items, colorClass, onView, onCategoryClick }: any) => {
    const rowRef = useRef<HTMLDivElement>(null);

    if (!items || items.length === 0) return null;

    // 🎨 Helper: Random Gradient for Blank Cards
    const getGradient = (id: string) => {
        const variants = colorClass === 'emerald'
            ? ['from-emerald-900/40 to-black', 'from-green-900/40 to-emerald-950', 'from-teal-900/40 to-black']
            : ['from-fuchsia-900/40 to-black', 'from-purple-900/40 to-fuchsia-950', 'from-pink-900/40 to-black'];
        return variants[id.charCodeAt(0) % variants.length];
    };

    // 🖱️ Helper: Scroll Function
    ...
    return (
        <div className="mb-10 animate-[fade-in_0.5s] relative group/row">
            {/* ✨ Title Header */}
            <div className="flex items-center gap-3 mb-4 px-4">
                <div className={`p-1.5 rounded-lg ${colorClass === 'emerald' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-fuchsia-500/20 text-fuchsia-400'}`}>
                    {title.includes('Pinned') ? <Pin size={16} /> : <Layers size={16} />}
                </div>
                <h3 className={`text-lg font-bold tracking-wider uppercase ${colorClass === 'emerald' ? 'text-emerald-100' : 'text-fuchsia-100'}`}>
                    {title}
                </h3>
                <div className={`h-[1px] flex-1 opacity-20 ${colorClass === 'emerald' ? 'bg-emerald-500' : 'bg-fuchsia-500'}`}></div>
            </div>

            {/* ⬅️ LEFT SCROLL BUTTON (Desktop Only) */}
            <button
                onClick={() => scroll('left')}
                className="absolute left-2 top-[60%] -translate-y-1/2 z-30 p-2 rounded-full bg-black/60 text-white backdrop-blur-md opacity-0 group-hover/row:opacity-100 transition-opacity hidden md:flex hover:bg-black/90 hover:scale-110 border border-white/10"
            >
                <ChevronLeft size={24} />
            </button>

            {/* 🌊 HORIZONTAL SCROLL AREA */}
            <div ref={rowRef} className="flex gap-4 overflow-x-auto pb-8 px-4 snap-x snap-mandatory scroll-smooth scrollbar-hide mask-fade-sides">
                {items.map((item: any) => (
                    <div
                        key={item.id}
                        onClick={() => onView(item)}
                        className={`
                            relative shrink-0 w-[280px] h-[170px] rounded-2xl border overflow-hidden cursor-pointer 
                            group transition-all duration-500 snap-center hover:-translate-y-2 hover:shadow-2xl
                            ${colorClass === 'emerald'
                                ? 'bg-emerald-950/20 border-emerald-500/20 hover:border-emerald-400/50 hover:shadow-emerald-900/20'
                                : 'bg-fuchsia-950/20 border-fuchsia-500/20 hover:border-fuchsia-400/50 hover:shadow-fuchsia-900/20'}
                        `}
                    >
                        {/* Background Layer */}
                        {item.image ? (
                            <>
                                <img src={item.image} className="absolute inset-0 w-full h-full object-cover opacity-50 group-hover:opacity-70 transition-all duration-700 group-hover:scale-110" loading="lazy" />
                                <div className="absolute inset-0 bg-gradient-to-t from-black via-black/40 to-transparent" />
                            </>
                        ) : (
                            <div className={`absolute inset-0 bg-gradient-to-br ${getGradient(item.id)}`}>
                                <div className="absolute top-0 right-0 p-4 opacity-10 transform rotate-12 group-hover:rotate-0 transition-transform duration-700">
                                    {item.type === 'video' ? <Video size={80} /> : <FileText size={80} />}
                                </div>
                        ...
                        <div className="absolute inset-0 p-5 flex flex-col justify-between z-10">
                            <div className="flex justify-between items-start">
                                <span className={`text-[10px] font-bold px-2 py-1 rounded border backdrop-blur-md uppercase tracking-wider ${colorClass === 'emerald' ? 'bg-emerald-900/40 border-emerald-500/30 text-emerald-300' : 'bg-fuchsia-900/40 border-fuchsia-500/30 text-fuchsia-300'}`}>{item.date}</span>
                                {item.isPinned && <Pin size={14} className="text-yellow-400 drop-shadow-lg" fill="currentColor" />}
                            </div>
                            <div>
                                <h4 className="text-white font-bold text-lg leading-tight line-clamp-2 drop-shadow-md mb-1 group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-gradient-to-r from-white to-gray-300 transition-all">{item.title}</h4>
                                {!item.image && <p className="text-xs text-white/50 line-clamp-2 font-serif italic">{item.content?.replace(/<[^>]*>/g, '').substring(0, 60)}...</p>}
                            </div>
                        </div>
                    </div>
                ))}
                ...
                <div
                    ...
                    className={`
                        shrink-0 w-[100px] h-[170px] rounded-2xl border border-dashed flex flex-col items-center justify-center gap-3
                        opacity-40 hover:opacity-100 cursor-pointer transition-all duration-300 snap-center hover:scale-105 hover:bg-white/5
                        ${colorClass === 'emerald' ? 'border-emerald-500 text-emerald-400' : 'border-fuchsia-500 text-fuchsia-400'}
                    `}
                >
                    <div className="p-3 rounded-full bg-white/5 shadow-lg"><ChevronRight size={24} /></div>
                    <span className="text-[10px] font-bold uppercase tracking-widest text-center px-2">View<br />All</span>
                </div>
            </div>

            {/* ➡️ RIGHT SCROLL BUTTON (Desktop Only) */}
            <button
                onClick={() => scroll('right')}
                className="absolute right-2 top-[60%] -translate-y-1/2 z-30 p-2 rounded-full bg-black/60 text-white backdrop-blur-md opacity-0 group-hover/row:opacity-100 transition-opacity hidden md:flex hover:bg-black/90 hover:scale-110 border border-white/10"
            >
...
// ==========================================
// 🌳 COMPONENT: TREE LEAF NODE (Link Tree)
// ==========================================
const TreeLeafNode = ({
...
export const SectorView: React.FC<SectorViewProps> = ({
    ...
    const handleOpenCategory = (categoryName: string) => {
        if (categoryName.includes('Pinned')) {
            setShowPinnedOnly(true);
            setViewMode('masonry'); // Switch to Grid
        } else {
            setSubjectFilter(categoryName);
            setViewMode('masonry'); // Switch to Grid
        }
    };
    ...
    const groupedItems = useMemo(() => {
        ...
        if (pinned.length > 0) groups['📌 Pinned / Urgent'] = pinned;

        // 2. Subject Groups
        filteredItems.filter(i => !i.isPinned).forEach(item => {
            const key = item.subject || '📢 General Updates';
    ...
    return (
        <div className={`w-full max-w-7xl mx-auto px-4 sm:px-6 pb-24 animate-[fade-in_0.3s_ease-out] ${isWizard ? 'scrollbar-wizard' : 'scrollbar-muggle'}`}>
            ...
            <div className="flex flex-col md:flex-row justify-between items-end gap-4 mb-8 border-b border-white/10 pb-4">
                <div className="flex-1 w-full md:w-auto">
                    <div className="flex items-center gap-4 mb-2">
                        ...
                        {/* ✨ BACK TO FOLDERS BUTTON (Visible inside a subject) */}
                        ...
                        {/* ✨ BACK TO HOME BUTTON (Safe Navigation) */}
                        {onBack && (
                            <button onClick={onBack} className={`flex items-center gap-2 px-4 py-2 rounded-full font-bold transition-all shadow-lg hover:scale-105 active:scale-95 ${isWizard ? 'bg-emerald-900/40 text-emerald-300 border border-emerald-500/30' : 'bg-fuchsia-900/40 text-fuchsia-300 border border-fuchsia-500/30'}`}>
                                <ArrowLeft size={18} />
            ...
            {/* 🔒🔒🔒 THE SWIMLANE GUARD: NOW OPEN FOR ALL IF viewMode is 'list' */}
            {!isLectures && viewMode === 'list' ? (
                <div className="w-full pb-24 animate-[fade-in_0.5s]">
                    {Object.keys(groupedItems).length === 0 ? (
                        <div className="text-center opacity-50 py-10">No items found.</div>
                    ) : (
                        Object.entries(groupedItems).map(([category, groupItems]) => (
                            <SwimlaneRow
                                ...
                                title={category}
                                items={groupItems}
                                colorClass={isWizard ? 'emerald' : 'fuchsia'}
                                onView={onViewItem}
                                onCategoryClick={handleOpenCategory} // Functional Category Click
                            />
                        ))
                    )}
                </div>
            ...
            {/* --- STANDARD GRID / FOLDER VIEWS --- */}
            {/* Render ONLY if NOT Lectures AND NOT List view */}
            {!isLectures && viewMode !== 'list' && (
                <>
                    {/* Folder View */}
                    {viewMode === 'folders' && !search && !dateFilter && !showPinnedOnly ? (
                        ...
                        // Standard Grid / List (Fallback for non-announcements)
                        <>
                            ...
                            {isResources && subjectFilter && filteredItems.some(i => i.type === 'link_tree') ? (
                                ...
                                <>
                                    {filteredItems.length === 0 ? (
                                        ...
                                        // ✅ GRID / MASONRY (Standard Cards - NOT List because List is now Swimlane)
                                        // Note: If user selected 'list', the block above catches it.
                                        // So this block is ONLY for grid/masonry.
                                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-[fade-in_0.3s]">
                                            {filteredItems.map(item => (
                                                <div
                                                    key={item.id}
                                                    draggable={isAdmin && isManualSort}
                                                    ...
                                                    className={`
                                                        relative rounded-xl border backdrop-blur-md group transition-all duration-300 cursor-pointer overflow-hidden select-none p-6 flex flex-col gap-4 hover:-translate-y-1 h-full
                                                        ...
                                                        ${item.isPinned ? (isWizard ? 'border-l-4 border-l-yellow-500' : 'border-l-4 border-l-yellow-500') : ''}
                                                ...
                                                >
                                                    ...
                                                    {/* No Image Icon */}
                                                    {!item.image && (
                                                        <div className={`shrink-0 rounded-full flex items-center justify-center z-10 w-12 h-12 mb-2 ${isWizard ? 'bg-emerald-900/30 text-emerald-400' : 'bg-fuchsia-900/30 text-fuchsia-400'}`}>
                                                    ...
                                                    <div className="flex-1 min-w-0">
                                                        ...
                                                        <h4
                                                            className="font-bold leading-tight truncate text-white text-lg mb-2"
                                                        ...
                                                        {!(item.fileUrl && (item.fileUrl.startsWith('http') || item.fileUrl.startsWith('https'))) && <div className={`text-xs opacity-70 line-clamp-3 mt-1 text-zinc-300 ${isWizard ? 'font-wizard' : 'font-muggle'}`} style={{ color: item.style?.contentColor }} dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(item.content, { ALLOWED_TAGS: ['b', 'i', 'em', 'strong', 'u', 'span'], ALLOWED_ATTR: ['style', 'class'] }) }} />}
                                                        {(item.fileUrl && (item.fileUrl.startsWith('http') || item.fileUrl.startsWith('https'))) && (
                                                            <div className="mt-2 pt-2 border-t border-white/5 flex">
                                                                <a href={item.fileUrl} target="_blank" rel="noreferrer" onClick={(e) => e.stopPropagation()} className={`flex items-center justify-center gap-2 px-4 py-2 rounded font-bold text-xs uppercase tracking-widest transition-all hover:scale-[1.02] active:scale-95 shadow-lg ${isWizard ? 'bg-emerald-600 hover:bg-emerald-500 text-black' : 'bg-fuchsia-600 hover:bg-fuchsia-500 text-black'}`}>
